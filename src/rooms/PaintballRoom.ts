...
function resolvePos(cx: number, cz: number, rad: number) {
  ...
  for (const o of OBSTACLES) {
    ...
    if (x > minx && x < maxx && z > minz && z < maxz) {
      // overlap → push out along the shallowest axis
...
// Does the segment (from→to) hit an obstacle box (top view)? Cheap sampled test.
function blockedByObstacle(x: number, z: number) {
  for (const o of OBSTACLES) {
...
export class PaintballRoom extends Room {
  ...
  private _tick() {
    ...
    this.state.players.forEach((p: PBPlayer, sid: string) => {
      ...
      p.x = res.x; p.z = res.z; p.y = 0;
      p.moving = Math.hypot(inp.x, inp.z) > 0.05;
    });
